...
```{r Initial RBD identity matrix, fig.height = 5, fig.width = 6.5}
...
```{r Clade-wise differences in RBD sequences, fig.height = 1, fig.width = 1.5}
...
```{r Making the identity matrix of the ACE2 orthologs we were able to make, fig.height = 2, fig.width = 3}
...
```{r Of the ones we were able to make now looking at the actual sequences we tested, fig.height = 2, fig.width = 3}
...
```{r Looking at all of the bat ACE2s in NCBI, fig.height = 4, fig.width = 5}
...
For Pymol, this is the original view I was taking:
set_view (\
...
But perhaps it will be good to use this other view for looking at the interaction surface
set_view (\
...
```{r Incorporating the bat species range data}
